...
export function QuizPage() {
  ...
  const handleFiltersChange = useCallback(
    (category: string | null, difficulty: Difficulty | null) => {
      updateFilters(category, difficulty);
    },
    [updateFilters],
  );
    ...
    if (isPlaying || state === "loading") {
      return (
        <QuizPlaying
          ...
          loading={
            deferredLoadingWord || deferredCheckingAnswer || state === "loading"
          }
    ...
    if (isFinished) {
      return (
        <QuizFinished
          stats={context.stats}
          wordsCompleted={
            context.reinforceMode
              ? context.masteredCount
    ...
    <div className="app">
      <header className="header">
        ...
        <p className="header__subtitle">
          {isFinished ? "Quiz zakończony!" : "Ucz się słówek przez tłumaczenie"}
        </p>
      </header>
